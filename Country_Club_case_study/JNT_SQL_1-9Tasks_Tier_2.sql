...
SELECT f.name AS booking, CONCAT(m.firstname,' ',m.surname) AS member_name, 
...
SELECT subquery.name AS booking, CONCAT(m.firstname,' ',m.surname) AS member_name, subquery.total_cost
	
FROM (SELECT Facilities.name, memid, starttime,
...
WHERE DATE(starttime) = '2012-09-14' AND subquery.total_cost > 30
ORDER BY total_cost DESC;
